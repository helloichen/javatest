		...
		<table border="1">
			<tr>
				...
				<td><select id="doorId" name="doorId">
						<%-- <c:forEach items="${ door }">
							<option value="${ order.doorId }">
								<c:if test="${ order.doorId==door.id }">${ door.name }</c:if>
							</option>
						</c:forEach> --%>+

						<c:forEach items="${ doorList }" var="door">
							<option ${ order.doorId==door.id?"selected":"" }
			...
			<tr>
				...
				<td><input type="text" name="payTime"
					value='<fmt:formatDate
						value="${ order.payTime }" pattern="yyyy/MM/dd HH:mm:ss" />' /></td>
			</tr>
			<tr>
				<td>支付方式</td>
